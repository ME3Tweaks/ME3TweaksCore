...
namespace ME3TweaksCore.ME3Tweaks.M3Merge.Game2Email
{
    public class ME2EmailMerge
    {
        ...
        public static bool NeedsMergedGame2(GameTarget target)
        {
        ...
        public static string RunGame2EmailMerge(M3MergeDLC mergeDLC, Action<string> progressChanged)
        {
            ...
            if (pccFile is null) return LC.GetString(LC.string_emailsNotMergedMessagesFileNotFound);
            ...
            {
                throw new Exception(LC.GetString(LC.string_game2EmailMergeWrongGame));
            }
            ...
            foreach (var emailMod in emailInfos)
            {
                ...
                {
                    MLog.Information($@"Merging email {email.EmailName}");
                    ...
                    progressChanged?.Invoke($@"{LC.GetString(LC.string_synchronizingEmails)} {(int)(done++ * 100.0f / totalEmails)}%");
                }
            }
